...
function buildClient() {
  return new SESClient({
    ...
    // env vars locally → IAM role in production (Lambda, EC2, ECS)
...
<body style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#18174c">
  ...
  <table style="border-collapse:collapse;width:100%">
    <tr><td style="padding:6px 12px 6px 0;font-weight:bold;white-space:nowrap">Namn</td><td style="padding:6px 0">${escapeHtml(safeName)}</td></tr>
    <tr><td style="padding:6px 12px 6px 0;font-weight:bold;white-space:nowrap">E-post</td><td style="padding:6px 0"><a href="mailto:${encodeURIComponent(safeEmail)}">${escapeHtml(safeEmail)}</a></td></tr>
    <tr><td style="padding:6px 12px 6px 0;font-weight:bold;white-space:nowrap">Telefon</td><td style="padding:6px 0">${escapeHtml(safePhone) || '–'}</td></tr>
    <tr><td style="padding:6px 12px 6px 0;font-weight:bold;white-space:nowrap">Adress</td><td style="padding:6px 0">${escapeHtml(safeAddress) || '–'}</td></tr>
  </table>
  <h3 style="margin-bottom:8px">Meddelande</h3>
  <p style="white-space:pre-wrap;background:#f5f5f5;padding:16px;border-radius:8px">${escapeHtml(message.trim())}</p>
</body>
</html>`
